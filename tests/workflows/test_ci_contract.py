...
class TestCIWorkflowContract:
    ...
    def setup_method(self):
        ...
        self.contract_file = (
            self.repo_root
            / "specs"
            / "002-github-workflows"
            / "contracts"
            / "ci-workflow-contract.yml"
        )
    ...
    def test_workflow_file_exists(self):
        ...
        ), f"CI workflow file not found at {self.workflow_file}"
    ...
    def test_workflow_name_matches_contract(self):
        ...
        ), f"Workflow name '{actual_name}' does not match contract '{expected_name}'"
    ...
    def test_workflow_triggers_push_events(self):
        ...
        if isinstance(push_config, dict) and "branches" in push_config:
            ...
            ), "Push trigger must specify branch configuration"
    ...
    def test_workflow_triggers_pull_request_events(self):
        ...
        ), "Workflow must trigger on pull request events"
    ...
    def test_workflow_has_backend_tests_job(self):
        ...
        for job_name, job_config in jobs.items():
            if "backend" in job_name.lower() or "test" in job_name.lower():
                ...
                has_test_step = any(
                    "test" in str(step).lower() or "pytest" in str(step).lower()
                    for step in steps
                )
                assert (
                    has_test_step
                ), f"Backend job '{job_name}' must include test execution steps"
                break
    ...
    def test_workflow_has_frontend_tests_job(self):
        ...
        for job_name, job_config in jobs.items():
            if "frontend" in job_name.lower() or (
                "test" in job_name.lower() and "npm" in str(job_config)
            ):
                frontend_job_found = True
                # Check if job includes npm test steps
                steps = job_config.get("steps", [])
                has_npm_test = any(
                    "npm test" in str(step) or "npm run test" in str(step)
                    for step in steps
                )
                assert (
                    has_npm_test
                ), f"Frontend job '{job_name}' must include npm test execution"
                break
    ...
    def test_workflow_has_security_scan_job(self):
        ...
        for job_name, job_config in jobs.items():
            ...
            steps = job_config.get("steps", [])
            has_security_step = any(
                "security" in str(step).lower() or "vulnerability" in str(step).lower()
                for step in steps
            )
    ...
    def test_workflow_has_docker_build_job(self):
        ...
        for job_name, job_config in jobs.items():
            if "docker" in job_name.lower() or "build" in job_name.lower():
                ...
                assert (
                    has_docker_step
                ), f"Docker job '{job_name}' must include Docker build steps"
                break
