...
class Substitute(Base):
    ...
    substitute_component_id = Column(
        String, ForeignKey("components.id"), nullable=False
    )

    # Relationship details
    notes = Column(Text, nullable=True)  # Notes about the substitution
    compatibility_rating = Column(
        String(20), nullable=True
    )  # perfect, good, acceptable, etc.
    ...
    component = relationship(
        "Component", foreign_keys=[component_id], back_populates="substitute_for"
    )
    substitute_component = relationship(
        "Component",
        foreign_keys=[substitute_component_id],
        back_populates="substitute_options",
    )
