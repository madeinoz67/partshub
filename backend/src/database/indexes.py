...
def create_search_indexes(session: Session) -> None:
    ...
    try:
        ...
        session.commit()
        logger.info(f"Successfully created {len(indexes_to_create)} database indexes for search optimization")
    ...
    try:
        for query_info in performance_queries:
            ...
            try:
                ...
                uses_index = "USING INDEX" in plan_text.upper() or "INDEX" in plan_text.upper()
...
def optimize_database_for_search(session: Session) -> dict:
    ...
    try:
        ...
        optimized_queries = sum(1 for q in results["performance_analysis"].values()
                              if q.get("optimized", False))
        total_queries = len(results["performance_analysis"])

        if optimized_queries == total_queries:
            results["recommendations"].append("✅ All queries are optimized with indexes")
        else:
            results["recommendations"].append(f"⚠️ {total_queries - optimized_queries} queries may need optimization")
        ...
        if component_count > 1000:
            results["recommendations"].append("📊 Large component table detected - indexes are critical for performance")
        if component_count > 10000:
            results["recommendations"].append("🚀 Consider implementing database partitioning for very large datasets")

        results["recommendations"].append(f"📈 Database contains {component_count} components with {stats.get('total_indexes', 0)} search indexes")
