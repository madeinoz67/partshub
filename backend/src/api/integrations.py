...
class ComponentImportRequest(BaseModel):
    ...
    download_attachments: dict[
        str, bool
    ] | None = None  # {'datasheet': True, 'image': True}
...
async def search_components_from_providers(
    ...
    try:
        results = await provider_service.search_components(
            query=request.query, limit=request.limit, providers=request.providers
        )
...
async def search_components_by_provider_sku(
    ...
    try:
        results = await provider_service.search_by_provider_sku(
            provider_sku=request.provider_sku, providers=request.providers
        )
        ...
        if download_attachments and result.get("imported_components"):
            try:
                ...
                for import_detail in import_results:
                    if import_detail.get("status") == "imported" and import_detail.get(
                        "search_result"
                    ):
    ...
    try:
        ...
        if request.download_attachments and result.get("imported_components"):
            try:
                ...
                for component_data in request.components:
                    if "search_result" in component_data:
                        ...
                        for imported in result.get("import_details", []):
                            if imported.get("status") == "imported" and imported.get(
                                "part_number"
                ...
                if components_with_results:
                    attachment_results = await provider_attachment_service.download_attachments_for_components(
                        components_with_results, request.download_attachments
                    )
                    result["attachment_downloads"] = attachment_results
    ...
    try:
        ...
        try:
            ...
            if not provider_data:
                raise HTTPException(
                    status_code=400,
                    detail="No provider data available for this component",
                )
...
async def export_kicad_library(
    ...
    try:
        if request.component_ids:
            ...
            try:
                components = (
                    ...
                    .filter(Component.id.in_(request.component_ids))
                    .all()
                )
            finally:
                session.close()

            zip_data = kicad_service.export_component_library(
                components, request.library_name
            )

        elif request.category_id:
            zip_data = await kicad_service.export_components_by_category(
                request.category_id
            )

        elif request.manufacturer_name:
            zip_data = await kicad_service.export_components_by_manufacturer(
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={request.library_name}.zip"
            },
        )
...
async def get_kicad_export_info():
    ...
    try:
        ...
        return {"export": export_info, "templates": templates_info}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
