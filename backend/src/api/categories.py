...
def get_categories(
    ...
    if hierarchy:
        ...
        root_categories = db.query(Category).filter(Category.parent_id.is_(None)).order_by(Category.sort_order, Category.name).all()
        ...
        if not include_empty:
            categories = [cat for cat in categories if cat.get_component_count(include_children=False) > 0]
...
def update_category(
    ...
    if category_data.name:
        existing = db.query(Category).filter(
            Category.name == category_data.name,
            Category.parent_id == (category_data.parent_id if category_data.parent_id is not None else category.parent_id),
...
def delete_category(
    ...
    if component_count > 0:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail="Invalid reassign_to category ID format")
        ...
        if reassign_to == category_id:
            raise HTTPException(status_code=400, detail="Cannot reassign components to the same category being deleted")
...
def get_category_components(
    category_id: str,
    include_children: bool = Query(False, description="Include components from child categories"),
    limit: int = Query(50, ge=1, le=1000, description="Maximum number of components to return"),
    ...
    if include_children:
        ...
        components_query = db.query(Component).filter(Component.category_id.in_(category_ids))
    else:
        components_query = db.query(Component).filter(Component.category_id == category_id)

    # Get total count
    total_count = components_query.count()

    # Apply pagination and get components
    components = components_query.order_by(Component.name).offset(offset).limit(limit).all()
