...
class TestStorageBulkContract:
    ...
    def test_bulk_create_storage_hierarchy_with_jwt(self, client: TestClient):
        ...
        response = client.post("/api/v1/storage-locations/bulk-create", json=bulk_data, headers=headers)
        ...
        if response.status_code == 201:
            ...
            assert drawer1["location_hierarchy"] == "Workshop/Electronics Cabinet A/Drawer 1"
    ...
    def test_bulk_create_validation_errors(self, client: TestClient):
        ...
        response = client.post("/api/v1/storage-locations/bulk-create", json=invalid_data, headers=headers)
        assert response.status_code == 422

        # Empty locations array
        empty_data = {"locations": []}
        response = client.post("/api/v1/storage-locations/bulk-create", json=empty_data, headers=headers)
        assert response.status_code == 422
        ...
        response = client.post("/api/v1/storage-locations/bulk-create", json=incomplete_data, headers=headers)
        assert response.status_code == 422
    ...
    def test_bulk_create_transaction_rollback(self, client: TestClient):
        ...
        if list_response.status_code == 200:
            ...
            valid_location_exists = any(loc["name"] == "Valid Location" for loc in locations)
