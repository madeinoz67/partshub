...
class TestBOMContract:
    ...
    def test_generate_component_list_bom_empty_components(self, client: TestClient):
        ...
        data = {"components": [], "include_provider_data": True}
    ...
    def test_generate_component_list_bom_invalid_component_structure(
    ...
    def test_export_bom_missing_both_project_and_components(self, client: TestClient):
        ...
        data = {"export_format": "csv", "include_provider_data": True}

        response = client.post("/api/v1/bom/export", json=data)
        # May fail due to service dependencies, but should validate structure
    ...
    def test_export_bom_with_project_id(self, client: TestClient):
        ...
        response = client.post("/api/v1/bom/export", json=data)
        # Valid request structure, may fail on service dependencies or validation
    ...
    def test_export_project_bom_valid_formats(self, client: TestClient):
        ...
        for format_type in formats:
            response = client.get(
                f"/api/v1/bom/project/{project_id}/export/{format_type}"
            )
            # Valid format, may fail on service
            assert response.status_code in [200, 400, 404, 500]
    ...
    def test_export_project_bom_invalid_format(self, client: TestClient):
        ...
        response = client.get(
            f"/api/v1/bom/project/{project_id}/export/{invalid_format}"
        )
        # Should validate format, but may fail due to service dependencies
        assert response.status_code in [400, 500]
    ...
    def test_bom_provider_data_parameter(
        self, client: TestClient, include_provider_data
    ):
    ...
    def test_bom_refresh_provider_data_parameter(
        self, client: TestClient, refresh_provider_data
    ):
    ...
    def test_component_list_bom_quantity_types(self, client: TestClient):
        ...
        data = {"components": test_cases, "include_provider_data": False}
